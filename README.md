...
````
git clone https://github.com/Timur-Razzakov/Notification-service.git
````
...
1. Склонировать репозиторий с Github
```
git clone https://github.com/Timur-Razzakov/Notification-service.
```
2. Перейти в директорию проекта
...
```
***
```http://0.0.0.0:8000/v1/``` - список API

```http://0.0.0.0:8000/clients/``` - клиенты

```http://0.0.0.0:8000//mailings/``` - рассылки

```http://0.0.0.0:8000/mailings/get_total_info/``` - общая статистика по всем рассылкам

```http://0.0.0.0:8000/mailings/<pk>/get_message/``` - детальная статистика по конкретной рассылке
...
## Что из дополнительного было выполнено
...
4. обеспечить интеграцию с внешним OAuth2 сервисом авторизации для административного интерфейса.
